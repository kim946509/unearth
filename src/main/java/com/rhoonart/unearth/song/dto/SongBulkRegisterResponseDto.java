    ...
    private static List<FailureInfo> convertToFailureInfoList(
        ...
        return failureDataList.stream()
                .map(failureData -> FailureInfo.builder()
                        .artistKo(failureData.csvData().getArtistKo())
                        .artistEn(failureData.csvData().getArtistEn())
                        .albumKo(failureData.csvData().getAlbumKo())
                        .albumEn(failureData.csvData().getAlbumEn())
                        .titleKo(failureData.csvData().getTitleKo())
                        .titleEn(failureData.csvData().getTitleEn())
                        .youtubeUrl(failureData.csvData().getYoutubeUrl())
                        .melonSongId(failureData.csvData().getMelonSongId())
                        .rightHolderName(failureData.csvData().getRightHolderName())
